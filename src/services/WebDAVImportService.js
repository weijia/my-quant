...
class DataConverter {
  ...
  static convertStockToStrategy(stock, webdavData) {
    ...
    const existingAdvanced = advancedStrategies.find(a => 
      (a.stockName === name || a.name === name) && 
      a.accountType === stock.accountType
    )
    ...
    const decreaseStrategies = conditionalStrategies
      .filter(c => (c.stockName === name || c.name === name) && 
  ...
  static convertAdvancedStrategy(advanced) {
    ...
    return {
      name: advanced.stockName || advanced.name || '',
  ...
  static formatMarketValue(value) {
    ...
    return this.parseNumber(value).toLocaleString('zh-CN')
  }
}
  ...
  async importFromWebDAV() {
    try {
      ...
      return {
        success: true,
        count: importedCount,
        message: `成功从 WebDAV 导入了 ${importedCount} 条策略`
      }
    } catch (error) {
  ...
  async importFromJSON(jsonData) {
    try {
      ...
      return {
        success: false,
        count: 0,
        message: '导入 JSON 数据失败: ' + error.message
      }
    }
  }
}
